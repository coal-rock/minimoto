...
class GrassManager:
    ...
    def place_tile(self, location, density, grass_options):
        ...
        loc = tuple(location)
        if loc not in self.grass_tiles:
            self.grass_tiles[loc] = GrassTile(self.tile_size, (loc[0] * self.tile_size, loc[1] * self.tile_size), density, grass_options, self.ga, self)
    ...
    def apply_force(self, location, radius, dropoff):
        ...
        for y in range(tile_range * 2 + 1):
            ry = grid_pos[1] + y - tile_range
            for x in range(tile_range * 2 + 1):
                rx = grid_pos[0] + x - tile_range
                pos = (rx, ry)
                if pos in self.grass_tiles:
                    self.grass_tiles[pos].apply_force(location, radius, dropoff)

    # apply a force based on a pygame mask
    def apply_mask_force(self, mask, location):
        location = (int(location[0]), int(location[1]))
        grid_pos = (int(location[0] // self.tile_size), int(location[1] // self.tile_size))
        mask_size = mask.get_size()
        tile_range_x = math.ceil(mask_size[0] / self.tile_size) + 1
        tile_range_y = math.ceil(mask_size[1] / self.tile_size) + 1
        ...
        for y in range(tile_range_y):
            ...
            for x in range(tile_range_x):
                rx = grid_pos[0] + x
                pos = (rx, ry)
                tile = grass_tiles.get(pos)
                if tile:
                    tile.apply_mask_force(mask, location)

    # an update and render combination function
    def update_render(self, surf, dt, offset=(0, 0), rot_function=None):
        surf_w, surf_h = surf.get_size()
        tile_size = self.tile_size
        width_tiles = int(surf_w // tile_size) + 1
        height_tiles = int(surf_h // tile_size) + 1
        base_x = int(offset[0] // tile_size)
        base_y = int(offset[1] // tile_size)

        padding = 3 # Optimize: only update near-screen tiles
        
        # 1. Update logic for near-screen tiles (including visible ones)
        render_list = []
        grass_tiles = self.grass_tiles
        
        for y in range(-padding, height_tiles + padding):
            ty = base_y + y
            for x in range(-padding, width_tiles + padding):
                tx = base_x + x
                pos = (tx, ty)
                tile = grass_tiles.get(pos)
                if tile:
                    ...
                    on_screen = (0 <= x < width_tiles and 0 <= y < height_tiles)
                    if on_screen:
                        render_list.append(tile)
                    else:
                        # Only update physics if near-screen but not visible
                        tile.update(dt)

        # 2. Render shadow if applicable
        if self.ground_shadow[0]:
            shadow_offset = (offset[0] - self.ground_shadow[3][0], offset[1] - self.ground_shadow[3][1])
            for tile in render_list:
                tile.render_shadow(surf, offset=shadow_offset)

        # 3. Render the grass tiles
        for tile in render_list:
...
class GrassAssets:
    def __init__(self, path, gm):
        self.gm = gm
        self.blades = []
        self.rotated_blades = {} # Cache for rotated blades
    ...
    def get_blade(self, blade_id, rotation):
        # Optimization: cache rotated blades
        rot_key = (blade_id, int(rotation))
        if rot_key in self.rotated_blades:
            return self.rotated_blades[rot_key]
        ...
        shade.set_alpha(int(shade_amt))
        rot_img.blit(shade, (0, 0))
        
        # Cache both image and its half-size for faster blitting
        rot_img_data = (rot_img, rot_img.get_width() // 2, rot_img.get_height() // 2)
        self.rotated_blades[rot_key] = rot_img_data
        return rot_img_data

    def render_blade(self, surf, blade_id, location, rotation):
        rot_img, half_w, half_h = self.get_blade(blade_id, rotation)
        # render the blade
        surf.blit(rot_img, (location[0] - half_w, location[1] - half_h))
...
class GrassTile:
    ...
    def apply_force(self, force_point, force_radius, force_dropoff):
        if not self.custom_blade_data:
            # Create a copy of blades data
            self.custom_blade_data = [list(b) for b in self.blades]
        ...
        for i, blade in enumerate(self.blades):
            ...
            if force > 0:
                direction = 1 if force_point[0] > (self.loc[0] + blade[0][0]) else -1
                # apply force
                amount = direction * force * 90
                current_bend = self.custom_blade_data[i][2] - blade[2]
                if direction > 0:
                    if amount > current_bend:
                        self.custom_blade_data[i][2] = blade[2] + amount
                else:
                    if amount < current_bend:
                        self.custom_blade_data[i][2] = blade[2] + amount

    def apply_mask_force(self, mask, mask_offset):
        mask_size = mask.get_size()
        mask_center_x = mask_offset[0] + mask_size[0] / 2
        
        # Cache locals for performance
        loc_x, loc_y = self.loc
        m_off_x, m_off_y = mask_offset
        m_w, m_h = mask_size

        for i, blade in enumerate(self.blades):
            # world pos of blade
            world_pos_x = loc_x + blade[0][0]
            world_pos_y = loc_y + blade[0][1]
            
            # pos relative to mask
            rel_x = int(world_pos_x - m_off_x)
            rel_y = int(world_pos_y - m_off_y)
            ...
            if 0 <= rel_x < m_w and 0 <= rel_y < m_h:
                if mask.get_at((rel_x, rel_y)):
                    ...
                    direction = 1 if world_pos_x > mask_center_x else -1
                    amount = direction * 90
    ...
    def set_rotation(self, rotation):
        if self.master_rotation != rotation:
    ...
    def render_tile(self, render_shadow=False):
        ...
        if render_shadow:
            ...
            for blade in self.blades:
                pygame.draw.circle(shadow_surf, self.gm.ground_shadow[1], (int(blade[0][0] + self.padding), int(blade[0][1] + self.padding)), self.gm.ground_shadow[0])
    ...
    def update(self, dt):
        ...
        if self.custom_blade_data:
            matching = True
            stiffness_dt = self.gm.stiffness * dt
            for i, blade in enumerate(self.custom_blade_data):
                blade[2] = normalize(blade[2], stiffness_dt, self.blades[i][2])
    ...
    def render(self, surf, dt, offset=(0, 0)):
        ...
        if self.custom_blade_data:
            x_pos = self.loc[0] - offset[0]
            y_pos = self.loc[1] - offset[1]
            for blade in self.custom_blade_data:
                self.ga.render_blade(surf, blade[1], (blade[0][0] + x_pos, blade[0][1] + y_pos), max(-90, min(90, blade[2] + self.true_rotation)))
            ...
            if (self.render_data not in self.gm.grass_cache):
                if self.gm.ground_shadow[0] and (self.base_id not in self.gm.shadow_cache):
